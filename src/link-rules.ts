...
function dedupeKey(url: string, type: string): string {
  const stripQuery = (u: string) => u.split('?')[0]
  if (type === 'bilibili') {
    const m = url.match(/bilibili\.com\/video\/([ab]v[0-9a-zA-Z_-]+)/i)
    if (m) return `bilibili:${m[1].toLowerCase()}`
  }
  if (type === 'douyin') {
  ...
  if (type === 'kuaishou') {
    const m = url.match(/kuaishou\.com\/short-video\/([0-9a-zA-Z_/-]+)/)
    if (m) return `kuaishou:${m[1]}`
  }
  if (type === 'xiaohongshu') {
    const m = url.match(/xiaohongshu\.com\/(?:discovery\/item|explore|board)\/([0-9a-zA-Z_-]+)/)
    if (m) return `xiaohongshu:${m[1]}`
  }
  if (type === 'weibo') {
    const m = url.match(/weibo\.com\/\d+\/([0-9a-zA-Z_-]+)/) || url.match(/weibo\.cn\/\S+\/(\d+)/)
    if (m) return `weibo:${m[1]}`
  }
  if (type === 'youtube') {
    const m = url.match(/[?&]v=([a-zA-Z0-9_-]{11})/) || url.match(/youtu\.be\/([a-zA-Z0-9_-]{11})/) || url.match(/youtube\.com\/shorts\/([a-zA-Z0-9_-]+)/)
  ...
  if (type === 'instagram') {
    const m = url.match(/instagram\.com\/(?:p|reel|share\/(?:reel|p))\/([0-9a-zA-Z_-]+)/)
    if (m) return `instagram:${m[1]}`
  }
  if (type === 'acfun') {
    const m = url.match(/acfun\.cn\/v\/ac(\d+)/)
    if (m) return `acfun:${m[1]}`
  }
  if (type === 'zhihu') {
    const m = url.match(/zhihu\.com\/video\/(\d+)/) || url.match(/zhihu\.com\/question\/(\d+)\/answer\/(\d+)/) || url.match(/zhihu\.com\/zvideo\/(\d+)/)
    if (m) return `zhihu:${m.slice(1).join(':')}`
  }
  if (type === 'xigua') {
    const m = url.match(/ixigua\.com\/(\d+)/)
    if (m) return `xigua:${m[1]}`
  }
  if (type === 'haokan') {
    const m = url.match(/haokan\.baidu\.com\/v\?vid=([0-9a-zA-Z_-]+)/)
    if (m) return `haokan:${m[1]}`
  }
  if (type === 'meipai') {
    const m = url.match(/meipai\.com\/media\/(\d+)/)
    if (m) return `meipai:${m[1]}`
  }
...
export function linkTypeParser(content: string, customRules: { pattern: RegExp; type: string }[]): LinkMatch[] {
  ...
  for (const rule of allRules) {
    ...
    while ((match = rule.pattern.exec(content)) !== null) {
      ...
      url = cleanUrl(url)
      if (!url) continue
      const key = dedupeKey(url, rule.type)
...
export function extractAllUrlsFromMessage(session: any, customRules: { pattern: RegExp; type: string }[]): LinkMatch[] {
  ...
  for (const link of matchedLinks) {
    ...
    const key = dedupeKey(cleaned, link.type)
    if (!seenUrls.has(key)) {
      seenUrls.add(key)
