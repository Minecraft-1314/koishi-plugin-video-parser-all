...
const PLATFORM_KEYWORDS = {
    bilibili: ['bilibili', 'b23', 'B站', 'www.bilibili.com', 'm.bilibili.com', '哔哩哔哩', 'QQ小程序', '小程序'],
    kuaishou: ['kuaishou', '快手', 'v.kuaishou.com', 'www.kuaishou.com', 'kwimgs.com', '快手极速版', '快手小程序'],
    xiaohongshu: ['xiaohongshu', '小红书', 'xhslink.com', 'xiaohongshu.com', 'xhscdn.com', '小红书小程序'],
    weibo: ['weibo', '微博', 'weibo.com', 'video.weibo.com', 'svproxy.168299.xyz', '微博小程序'],
    toutiao: ['toutiao', '今日头条', 'm.toutiao.com', 'toutiao.com', 'ixigua.com', '西瓜视频'],
    pipigx: ['pipigx', '皮皮搞笑', 'h5.pipigx.com', 'ippzone.com', '皮皮搞笑小程序'],
    pipixia: ['pipixia', '皮皮虾', 'h5.pipix.com', 'ppxsign.byteimg.com', '皮皮虾小程序'],
    douyin: ['douyin', '抖音', 'v.douyin.com', 'douyinpic.com', 'douyinvod.com', '抖音小程序'],
    zuiyou: ['zuiyou', '最右', 'xiaochuankeji.cn', 'izuiyou.com', '最右小程序']
};
const API_CONFIG = {
...
async function downloadVideoWithThreads(url, filename) {
    return new Promise((resolve, reject) => {
        ...
        const worker = new Worker(__filename, { workerData: { url, filePath } });
...
function parseData(data, maxDescLength, platform) {
    ...
    else if (data.image && Array.isArray(data.image)) images = data.image;
    let video = '';
    if (platform === 'douyin') {
    ...
    } else if (platform === 'pipigx') {
        video = data.video || data.url || '';
    } else if (platform === 'zuiyou') {
        video = data.url || '';
    } else {
        video = data.url || data.videos?.[0]?.url || data.video_backup?.[0]?.url || data.video || '';
    }
...
export function apply(ctx, config) {
    ...
    async function parse(url) {
        ...
        if (platform !== 'toutiao' && platform !== 'zuiyou') {
        ...
        for (let apiIndex = 0; apiIndex < platformApis.length; apiIndex++) {
            ...
            for (let retry = 0; retry <= config.retryTimes; retry++) {
                try {
                    ...
                    if ((res.data.code === 200 || res.data.code === 0) && (res.data.data || (platform === 'kuaishou' && res.data.image) || (platform === 'pipigx' && res.data.data) || (platform === 'zuiyou' && res.data.data))) {
                        let parseResult = null;
                        if (platform === 'kuaishou' && res.data.image && !res.data.data) {
    ...
    ctx.command('parse <url>', '手动解析视频链接')
        ...
        if (urls.length === 0) return '不支持该链接';
